...
⬜ **동기화 검증 보류.** Apple Developer 계정 + 같은 iCloud 계정의 실기기 2대가 필요하다.
✅ **모델 형태는 검증됨** (아래).

### S2-a. 모델 제약 검증 (2026-08-10, iPhone 17 시뮬레이터)

`Soozip/Spikes/S2_CloudKitProbe.swift`를 만들어 CloudKit 제약을 지킨 모델로 앱을 띄웠다.

| 항목 | 결과 |
|---|---|
| 전 속성 기본값/optional · unique 없음 · 관계 양방향 optional | 컴파일 통과 |
| `ModelContainer` 초기화 | **성공** (크래시 없음) |
| `@Query` 조회 · 화면 렌더 | 정상 |

**함정 하나 — `@Model`이 이니셜라이저를 직접 요구한다.** 모든 속성에 기본값이 있어도 매크로가 만들어 주지 않는다.

```
error: @Model requires an initializer be provided for 'ProbeCollection'
```

구 Phase 0 계획서(`01-phase0-setup-spikes.md`)에 실려 있던 모델 코드가 이 때문에 컴파일되지 않았다. `init() {}`를 명시해 해결했다. **Phase 1의 정식 `Collection`·`Canvas`·`CanvasPhoto`도 같은 요구를 받는다** — v4 §7의 모델 스니펫에는 이니셜라이저가 없으므로 그대로 옮기면 같은 곳에서 막힌다.

### S2-b. 남은 측정 (실기기 2대)

| 검증 | 절차 | 통과 기준 |
|---|---|---|
| 양방향 동기화 | A에서 [추가] → B에서 확인, 그 반대도 | 30초 내 반영 |
| 관계 동기화 | A에서 캔버스 추가 → B에서 개수 확인 | 개수 일치 |
| **초안 미동기화** | A에서 [초안 폴더 만들기] → 60초 뒤 B에서 [다시 확인] | **B가 "없음"** |
| 로컬 모드 | 한 대를 iCloud 로그아웃 → 앱 사용 | 크래시 없이 로컬 동작 |

**초안 미동기화가 Phase 2 설계 전체의 전제다**(v4 §6.2). 이것이 뒤집히면 초안을 SwiftData에 넣는 구조로 되돌아가야 하므로, **Phase 2 승격 트랜잭션에 착수하기 전에 반드시 닫는다.**
