...
@Test func 극단_비율의_레이어가_붕괴해도_결과는_유한하다() {
    ...
    // 하려면 붕괴 검출이 필요하다. **가드 전체를 지우는 변이는 이 테스트와
    // `반대_비율로_붕괴해도_결과는_유한하다`를 함께 죽인다. 가드의 일부 조건만
    // 지우는 변이는 이 테스트만 죽인다** — 이 테스트는 4필드 중 size.width·
    // center.x 2개만 비유한이라 나머지 두 조건이 없어져도 관측되지만,
    // `반대_비율…`은 네 필드가 전부 비유한이라 어느 하나만 남아도 가드가
    // 발동해 통과한다(그 테스트의 킬셋은 이 테스트에 포함되며 새로 죽이는
    // 변이는 없다 — 그래도 남기는 이유는 그 테스트 doc 참조).
    //
...
@Test func 드래그_좌표나_리사이즈_한계값이_비유한이면_결과가_유한하다() {
    ...
    //
    // 한계값 쪽(변형 5~8): **진입 가드가 없던 시절에는** 좌표 오염과 달리 결과를
    // NaN으로 오염시키지 않고 "한계를 조용히 무력화"했다 — 비유한 한계값이
    // 클램프의 비교를 전부 거짓으로 만들어 클램프 자체를 스킵시켰고, 결과는
    // 유한하지만 한계가 적용되지 않은 값이 났다. 그 시절의 실측(오케스트레이터가
    // `swiftc`로 재현):
    ...
    // `(2,1)`도 `(200898,100449)`도 완벽하게 유한했으므로 isFinite만으로는 이
    // 사고를 영영 관측할 수 없었다 — 결과가 "원본 프레임을 벗어나지 않는다"는
    // FR-5 후반부를 직접 재는 등가 단언이 이 네 변형에서만 필요한 이유다. 그래서
    // worldPoint도 변형 1~4의 (700,300)이 아니라, 실제로 각 한계가 발동하는
    // 지점(하한: 대각 고정점 근접, 상한: 초원거리)으로 잡았다 — (700,300)은
    // 정상 한계에서도 클램프가 발동하지 않는 드래그라 한계 무력화를
    // 관측하지 못한다. **오늘은 진입 가드(위 `guard worldPoint.x.isFinite, …`
    // 참조)가 문 앞에서 비유한 한계값을 잡아 `self`로 후퇴시키므로 클램프
    // 자체에 닿지 않는다** — 아래 등가 단언은 그 진입 가드가 회귀 없이
    // 계속 문 앞에서 잡는다는 것을 잰다.
    //
    // ⚠️ **`.nan`과 `.infinity` 하한은 진입 가드가 없던 시절에는 서로 반대
    // 메커니즘으로 같은 결론(유한하고 원본과 같음)에 도달했다** — 위 "비교를
    // 전부 거짓으로 만들어 스킵시킨다"는 설명은 그 시절에도 `.nan`과 상한 쪽
    // (`.nan`·`.infinity` 둘 다)에만 맞았다. `.infinity` **하한**은
    // `shortSide < .infinity`가 **참**이 되어 클램프를 스킵이 아니라 오히려
    // **강제 발동**시켰다 — `k = ∞/shortSide = ∞`가 되어 `size (∞,∞)`를
    // 만들고, 뒤이은 코너 보정의 `∞ − ∞ = NaN`이 center를 오염시킨 뒤 결과
    // 유한성 가드가 원본으로 후퇴시켜 등가 단언을 통과시켰다(그 시절 실측:
    // 원본 (200,100) 그대로). 상한 `.nan`/`.infinity`는 대칭이 아니었다 — 둘
    // 다 `longSide > .nan`/`longSide > .infinity` 비교가 거짓이 되어 그대로
    // 스킵됐다.
    //
    // **오늘은 진입 가드가 `minShortSide.isFinite`를 검사하므로 `.infinity`
    // 하한은 `clamped`에 도달하지 못한다 — 이 변형(하한 = .infinity)은 두
    // 가드 중 하나만 남아도 통과한다(진입 가드만 지워도 결과 가드가 받고,
    // 결과 가드만 지워도 진입 가드가 앞선다). 어느 가드의 단독 증인도 아니다.**
    //
    // (참고: 변 드래그 8변형(`변_드래그도_...`, 아래)에서는 이 함수에 결과
    // 유한성 가드가 추가되기 전 하한 `.infinity`가 실제로 실패했었다
    // (`size (∞,∞)`) — 그 경로엔 결과 가드가 없었기 때문이다. 지금은 코너·
    // 변 드래그 둘 다 진입 가드가 있어 비유한 한계값을 문 앞에서 먼저 잡는다.)
    let frame = LayerFrame(center: Vec2(x: 500, y: 400),
                           size: Size2(width: 200, height: 100),
                           rotation: 0)
...
@Test func 반대_비율로_붕괴해도_결과는_유한하다() {
    ...
    // 오버플로우한다. 이 임계(작은 쪽·반대쪽 둘 다)의 정확한 값과 유도,
    // 정정 이력은 `LayerFrame.clamped` doc 참조 — 이 임계의 정확한 값은
    // 그 한 곳에만 적는다.
    //
    // 킬셋은 `극단_비율의_레이어가_붕괴해도_결과는_유한하다`에 포함되며 새로
    // 죽이는 변이는 없다 — 그 테스트는 4필드 중 size.width·center.x 2개만
    // 비유한이라 가드의 일부 조건만 지워도 관측되지만, 이 테스트는 네 필드가
    // 전부 비유한이라 어느 조건 하나만 남아도 가드가 발동해 통과하므로 더 약한
    // 증인이다. **그래도 남기는 이유**: 이 테스트는 상한 블록이 발동하지 않고
    // 하한 블록 단독으로 오버플로우하는 경로를 지나가는 유일한 테스트다
    // (`극단_비율…`은 상한→하한 순으로 둘 다 발동한다) — 킬셋이 아니라 경로
    // 커버리지가 존재 이유다.
    //
