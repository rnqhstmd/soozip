...
extension LayerFrame {
    ...
    /// (`swiftc`로 확인: 결과가 정상과 동일). 신규 테스트 두 건은 하한에 `.nan`·
    /// `.infinity`도 넘기지만, 진입 가드가 비유한 한계를 `clamped` 진입 전에 걸러
    /// 결과가 매개변수 유무와 무관하게 같아진다 — 그 변이를 죽일 테스트는 여전히
    /// 없다. **이름을 갈라 컴파일 단계에서 막는다.**
    ...
    public func resized(draggingCorner corner: Corner,
        ...
        //
        // ⚠️ **가드 전체를 지우는 변이는 `극단_비율의_레이어가_붕괴해도_결과는_유한하다`와
        // `반대_비율로_붕괴해도_결과는_유한하다` 둘 다 죽인다.** 가드의 일부 조건만
        // 지우는 변이는 `극단_비율…`만 죽인다 — 그 테스트는 4필드 중 size.width·
        // center.x 2개만 비유한이라 나머지 두 조건이 없어져도 관측되지만, `반대_비율…`은
        // 네 필드가 전부 비유한이라 어느 하나만 남아도 가드가 발동해 통과한다(그 테스트의
        // 킬셋은 `극단_비율…`에 포함되며 새로 죽이는 변이는 없다 — 그래도 남기는 이유는
        // 그 테스트 doc 참조). **변 드래그 경로에도 같은 형태의 결과 유한성 가드가
        // 있다**(아래 `resized(draggingEdge:)` 참조) — 다만 그 경로를 쓰는 테스트 4건이
        // 전부 유한 입력이거나 진입 가드에서 끝나, 이 가드를 지워도 빨강이 되는 테스트가
        // 현재 없다(증인 0건).
    ...
    public func resized(draggingEdge edge: Edge,
        ...
        //
        // 방어가 없을 때 실측(세 값은 서로 다른 프레임에서 나왔다 — 프레임을
        // 병기한다): 프레임 size (200,100)에서 `.right` 변을 `(.infinity, 400)`으로
        // 끌면 `size (NaN, 0)` · `center (NaN, NaN)`이 **public API 밖으로 나갔다.**
        // 프레임 size (50,100)에서 하한이 `.nan`이면 `(25, 100)`(하한 40 무력화),
        // 프레임 size (200,100)에서 상한이 `.nan`이면 `(99599, 100)`(상한 7680
        // 무력화)이 나갔다.
        //
    ...
    /// 계산하면 된다.
    ///
    /// **반대 방향(비율이 큰 쪽) 임계도 같은 하한 블록에서, 반대 경로로 나온다.** 비율이
    /// 크면 상한 블록이 먼저 발동해 `(maxLongSide, maxLongSide/비율)`을 만들고, 이어서
    /// 하한 블록이 `k = minShortSide/(maxLongSide/비율)`을 두 축에 곱한다 — 대수적으로
    /// `maxLongSide`가 소거되어 최종 결과는 `(minShortSide × 비율, minShortSide)`이
    /// 된다(위 "유일하게 결정된다"와 정합). 이 곱셈이 오버플로우하는 지점이 임계이므로
    /// `maxLongSide`와 무관하게 `Double.greatestFiniteMagnitude / shortSideFloor(40) =
    /// 4.4942328371557894e306`(`swiftc` 실측)이다. (참고: `EDITOR-7` 코드 리뷰 전에는
    /// 작은 쪽 임계가 `2.2249e-307`로 적혀 있었다 — 오기였다.)
    ///
    ...
    /// 없음"의 조건이 `(min ≥ 하한 ∨ min = 0) ∧ max ≤ 상한`으로 같다 — 하한 블록의
    /// `shortSide > 0` 단서 때문에 `(0, 100)`처럼 `min < 하한`이어도 발동하지 않는
    /// 경우가 포함된다. 무작위 20,000회 대조 불일치
